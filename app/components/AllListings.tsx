...
const Alllistings: React.FC<PropertyPageProps> = ({
  ...
  return (
    <section
      className={`max-w-screen-xl px-4 bg-white ${filteredProperties.length > 0 ? "md:py-20 py-16 " : "flex items-center justify-center min-h-96"}`}
    >
        ...
        {filteredProperties.length > 0 ? (
          <>
            ...
            <div className="grid grid-cols-1 w-full sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {filteredProperties.map((post: any) => (
                <div
                  key={post._id}
          ...
          <div className="flex flex-col items-center justify-center text-gray-800 text-center space-y-4">
            ...
            <p className="text-lg font-medium">No available property</p>
          </div>
        )}
      </div>
    </section>
